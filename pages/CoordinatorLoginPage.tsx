...
interface CoordinatorLoginPageProps {
  onCoordinatorLogin: (coordinator: Coordinator) => void;
  onNavigateToUser: () => void;
}

const CoordinatorLoginPage: React.FC<CoordinatorLoginPageProps> = ({ onCoordinatorLogin, onNavigateToUser }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await mockApi.loginCoordinator(username, password);

    if (response.success && response.coordinator) {
      onCoordinatorLogin(response.coordinator);
    } else {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-md p-8 space-y-8 bg-white rounded-lg shadow-lg">
        <div className="text-center">
          <h2 className="text-3xl font-bold text-gray-800">Coordinator Portal</h2>
          <p className="mt-2 text-gray-600">Log in to manage your assigned exam</p>
        </div>
